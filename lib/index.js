...
/**
 * provider 形态返回的 skill 必须自带 rank（宿主 @deepseek-ai/dsh-skill 的
 * validateCandidate 硬要求：`typeof rank !== 'number'` 直接抛 invalid rank，
 * 而错误会冒到用户面前）。250 与宿主给运行期 skill 的 RUNTIME_RANK 一致：
 * 排在项目/agents 目录（100/200）之后、自定义目录（300）之前。
 * 详见 docs/skill-injection.md。
 */
...
export function toDefinition(s) {
  const def = {
    name: s.name,
