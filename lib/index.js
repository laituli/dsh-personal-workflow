...
export function apply(ctx) {
  ctx.effect(() => {
    ...
    const svc = ctx.skills;
    const disposers = [];
    if (!skills.length) {
      ctx.logger?.warn?.('[dsh-personal-workflow] skills/ 为空，未注册任何 skill');
      return () => {};
    }
    if (svc && typeof svc.registerProvider === 'function') {
      ...
      ctx.logger?.warn?.('[dsh-personal-workflow] 未找到 ctx.skills 注册接口（register/registerProvider），本次未注入；见 docs/skill-injection.md');
    }
    return () => {
      for (const d of disposers) { try { d(); } catch (e) { /* 忽略 */ } }
      ctx.logger?.info?.('[dsh-personal-workflow] 已卸载（skill 注册随 fiber 移除）');
    };
